...
import { useThemeContext } from '../../authkit-theme-provider/authkit-theme-provider';
...
const Modal: React.FC<ModalProps> = ({
  ...
  const { isSignedIn, reset } = useSIWA();

  const wallet = pipeState?.provider;
  ...
  }, [chain, mobile, isSignedIn, context.options, context.resize]);
            ...
            <ControllerContainer>
              ...
              >
                <AnimatePresence>
                  ...
                  ) : context.route === routes.PROFILE && context.signInWithEthereum ? (
                    <>
                      {!isSignedIn && !context.options?.hideTooltips && (
                        ...
                        >
                          <SignInTooltip>{locales.signInWithEthereumScreen_tooltip}</SignInTooltip>
                        </motion.div>
                      )}
                      <SiwaButton
                        disabled={inTransition}
                        aria-label={locales.signInWithEthereumScreen_signedOut_heading}
                        key="siwaButton"
                        onClick={() => {
                          reset();
